...
if model is None:
    ...
    input_data = pd.DataFrame({
        ...
        'Passport': [passport],
        'PitchSatisfactionScore': [0], # This is not an input feature during prediction as it's a result of the pitch
        'OwnCar': [owncar],
    ...
    # These should ideally come from a saved preprocessor or be hardcoded based on known data
    ...
    # This list of columns should ideally be loaded from a saved feature list from training
    training_columns = [
    ...
    ] # This list needs to be exact from the model training script
    ...
    # --- Prediction Button ---
    if st.button("Predict Purchase"): # Check if the model is loaded before enabling the button
        if model:
            try:
